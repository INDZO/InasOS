 ...
 * flat black screen. The glow and grid layers are driven by wallpaper
 * intensity (0-100) and stay visible even at the lowest setting.
 */
export default function OSWallpaper() {
  const intensity = useDesktopStore((s) => s.wallpaperIntensity);
  const factor = intensity / 100;
  const glowOpacity = 0.32 + 0.68 * factor;
  const gridOpacity = 0.22 + 0.78 * factor;
